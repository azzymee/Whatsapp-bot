...
async function startSocket(sessionWasRestored = false) {
  ...
  if (usePairingCode && !pairingCodeRequested && !sessionWasRestored) {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      startSocket(sessionWasRestored).catch((err) => logger.error({ err }, 'Reconnect attempt failed'));
